...
async function purge(ctx, invocation) {
  ...
  // 【关键】这里**绝不能**把 id 移出归档集合。
  // 归档是侧边栏里唯一让会话不显示的机制：移出 = 取消隐藏。早先就是这么写的，
  // 本意是「别留幽灵条目」，实际效果是十几个会话当场全部回到侧边栏。
  // 正确的做法是反过来 —— 确保它们**仍然**在归档里，删除文件不改变可见性。
  const keep = targets.filter((id) => !failed.some((f) => f.startsWith(id + '：')))
  try {
    await addToArchive(looked.reg, keep)
  } catch {
    // 加不进去也不影响已删的文件；最坏情况是它已经在归档里了
  }
  ...
  bits.push('它们仍保持归档状态，不会回到侧边栏。')
  if (ghosts.length > 0) bits.push('另有 ' + ghosts.length + ' 个在磁盘上没有文件，也保持归档。')
